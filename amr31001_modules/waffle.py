...
def create_node(name: str) -> Node:
    """
    create a ROS2 rclpy node for the waffle
...
def stop(node: Node, publisher: Publisher):
    for i in range(5):
        publisher.publish(Twist())
    node.destroy_node()
    rclpy.shutdown()

def get_time(node: Node) -> float:
    [seconds, nanoseconds] = node.get_clock().now().seconds_nanoseconds()
    time = float(seconds) + float(nanoseconds)*1e-9
    return time
